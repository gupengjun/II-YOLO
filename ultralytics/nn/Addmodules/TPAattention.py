...
class DynamicTPA_v2(nn.Module):
    def __init__(self, c1, c2, max_rank):
        super(DynamicTPA_v2, self).__init__()
        self.max_rank = max_rank
        self.A = Conv(c1, max_rank, 1, act=False)
        self.B = Conv(c1, max_rank, 1, act=False)
        ...
        self.gate = nn.Sequential(
            ...
            nn.Conv2d(c1, max_rank, 1),      # 映射到 rank 维度
        )
    ...
    def forward(self, x):
        ...
        # rank 权重向量 (softmax 保证是概率分布)
        gate_logits = self.gate(x_pe).view(B, self.max_rank)        # [B, R]
        rank_weight = torch.softmax(gate_logits, dim=-1)            # [B, R]

        # 截断 rank
        a = self.A(x_pe)   # [B, R, H, W]
        b = self.B(x_pe)   # [B, R, H, W]

        # 外积：得到 [B, R, R, H, W]
        ab = torch.einsum('bihw,bjhw->bijhw', a, b)

        # 用 rank_weight 对秩进行加权
        weight_matrix = torch.einsum('bi,bj->bij', rank_weight, rank_weight)  # [B, R, R]
        weight_matrix = weight_matrix.view(B, self.max_rank, self.max_rank, 1, 1)
        ab = ab * weight_matrix

        ab = ab.reshape(B, self.max_rank * self.max_rank, H, W)
