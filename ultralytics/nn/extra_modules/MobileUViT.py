        ...
        self.block = nn.Sequential(
            *[nn.Sequential(
                Residual(nn.Sequential(
                    nn.Conv2d(ch_in, ch_in, kernel_size=(kernel, kernel), groups=ch_in,
...
class GlobalSparseAttn_v2(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, qk_scale=None,
                 attn_drop=0., proj_drop=0., sr_ratio=1.,
                 gamma_small=2.0, gamma_large=1.0, size_thresh=32, learnable_gamma=False):
        super().__init__()
        self.num_heads = num_heads
        head_dim = dim // num_heads
        ...
        if learnable_gamma:
            self.gamma_small = nn.Parameter(torch.tensor(gamma_small, dtype=torch.float32))
            self.gamma_large = nn.Parameter(torch.tensor(gamma_large, dtype=torch.float32))
        else:
            self.gamma_small = gamma_small
            self.gamma_large = gamma_large

    def forward(self, x, H: int, W: int, target_sizes=32 * 32):
        B, N, C = x.shape
        if self.sr > 1.:
            x = x.transpose(1, 2).reshape(B, C, H, W)
            x = self.sampler(x)
        ...
        if target_sizes is not None:
            if isinstance(target_sizes, int):
                target_sizes = [target_sizes] * B
            elif len(target_sizes) != B:
                target_sizes = [target_sizes[0]] * B
            gammas = []
            for size in target_sizes:
                if size < self.size_thresh:
                    gammas.append(self.gamma_small)
                else:
                    gammas.append(self.gamma_large)

            gammas = torch.tensor(gammas, device=x.device).view(B, 1, 1, 1)
            attn = attn ** gammas
